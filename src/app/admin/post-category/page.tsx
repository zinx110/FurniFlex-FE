...
const PostCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (categoryName.length === 0 || categoryName.length > 100) {
      setErrorMessage('Category name is required and should not exceed 100 characters.');
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/Categories`, categoryData);
  ...
  return (
    <div className="container mx-auto mt-8 p-4 max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="categoryName" className="block text-lg font-medium">Category Name</label>
          <input
            type="text"
            id="categoryName"
            value={categoryName}
