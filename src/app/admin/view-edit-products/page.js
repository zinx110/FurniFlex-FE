...
export const ViewEditProducts = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.name.length > 255) {
      validationErrors.name = 'Product name cannot exceed 255 characters.';
      isValid = false;
    }
    ...
    if (formData.discountedPrice <= 0) {
      validationErrors.discountedPrice = 'Discounted price must be a positive number.';
      isValid = false;
    }
    ...
    if (formData.imageFile) {
      ...
      const fileExtension = formData.imageFile.name.split('.').pop().toLowerCase();
      ...
      if (!validExtensions.includes(fileExtension)) {
        validationErrors.imageFile = 'Image must be a valid file type (jpg, jpeg, png, gif).';
        isValid = false;
      }
    }
    ...
    <div className="max-w-full h-auto mx-auto py-10 px-10 bg-white text-black">
          ...
          {products.map((product) => (
            <tr key={product.ProductId}>
              ...
              <td className="border px-4 py-2">
                <img src={`${process.env.NEXT_PUBLIC_API_BASE_URL}/Products/${product.ProductId}/image`} alt={product.Name} className="h-16 w-16 object-cover" />
              </td>
              <td className="border px-4 py-2">
                <button onClick={() => handleEditClick(product)} className="bg-blue-500 text-white px-4 py-2 mr-2 rounded">
                  Edit
                </button>
                <button
      ...
      {editingProduct && (
        <form onSubmit={handleSubmit} className="mb-10">
          ...
          {formData.imgUrl && <img src={formData.imgUrl} alt="Preview" className="h-32 w-32 object-cover mb-4" />}
